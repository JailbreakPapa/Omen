...
- Console (PS5/Xbox) `IToolchain` implementations. The team confirmed they have console
  SDK access, but actually writing those toolchains is deferred by their own choice
  ("worry about that later") — this spec adds the `Prospero`/`Xbox` platform enum members
  and stub `IPlatformSDK`/`IToolchain` registrations so the seam exists, but the real
  compiler/linker invocation is separate follow-up work.
...
- **Correction from implementation review:** `ActionGraphBuilder.Build()` produces exactly
  **one** link/archive action per call — every module's object files are folded into a
  single `allObjectFiles` list and linked into one output (`BuildLinkAction`,
  `ActionGraphBuilder.cs:216-280`). There is no way today for a module to be its own
  independently linked binary. This means "Modular" isn't a working alternative path
  either — it's simply unimplemented, same as "Monolithic". Both Gem flavors (B2, which
  need Static/Runtime/Editor as three separate binaries from one gem) and real monolithic
  linking (A4) are blocked on this, so it's promoted to its own task (A0) ahead of both.
...
### A0. Independently-linked modules

New nullable `ModuleRules.BinaryType` (a `TargetType?`). When set, `ActionGraphBuilder`
gives that module its own archive/link action producing its own output artifact instead
of folding its objects into the target's aggregate link; dependents link against that
artifact (its import lib, for a Windows shared library) rather than absorbing its objects
directly. When unset (every module today), behavior is unchanged. This is the prerequisite
both A4 and B2 build on.
...
Built on A0. `ActionGraphBuilder` reads `TargetRules.LinkType`:
- `Modular`: a module with `BinaryType` set links independently (via A0), as intended.
- `Monolithic`: any module whose `BinaryType` would otherwise make it an independent
  shared library is instead folded into the aggregate archive/link like an unset-`BinaryType`
  module, and one final link produces the target binary. A generated `StaticModuleRegistration.g.cpp` (written to
